...
fileParser = catMaybes <$> many (spaces *> (commented <|> a_case) <* endOfLineAndFile)
    where commented = pure Nothing <* (try $ string "--") <* manyTill anyChar (lookAhead endOfLineAndFile)
          a_case    = Just <$> aCase
